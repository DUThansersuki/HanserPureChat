...
贴图”的材质保留 PMX 原始 diffuse，清除 `MMDLoader` 由 PMX ambient 映射而来的
材质自发光，再使用暖色主光、弱冷色补光和极低强度半球环境光。贴图输入和
渲染输出各执行一次 sRGB 转换，不启用额外 Gamma 或 ToneMapping。Canvas 仅使用中等
幅度的饱和度、对比度与亮度校正，不改变色相。需要整体提亮时优先调整最终画布
亮度，不重新抬高环境光或材质自发光，避免破坏肤色、明暗层次与透明边缘。

阴影柔化使用 `0.25` 的前侧补光，并将 MMD toon 渐变图从最近邻采样改为线性采样；
不增加环境光。角色后上方使用 `0.14` 的暖色定向轮廓光，仅用于分离猫耳、发梢和
肩部边缘。当前头发材质的 specular 为 `0`、shininess 为 `2`，且没有 sphere map；
头发亮部来自基础贴图和 toon 明暗，不额外更改镜面参数。
