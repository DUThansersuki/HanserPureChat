...
type GroupedChats = {
  today: ChatOverview[];
  yesterday: ChatOverview[];
  lastWeek: ChatOverview[];
  lastMonth: ChatOverview[];
  older: ChatOverview[];
};
...
const groupChatsByDate = (chats: ChatOverview[]): GroupedChats => {
  const now = new Date();
...
export function SidebarHistory({ user }: { user: { id: string } | undefined }) {
  ...
  if (error) {
    return (
      <SidebarGroup className="group-data-[collapsible=icon]:hidden">
        ...
        <SidebarGroupContent>
          <div className="flex items-center justify-between gap-2 px-3 py-2 text-[12px] text-sidebar-foreground/60">
            <span>暂时无法加载历史记录</span>
            <button
